...
func updateRedstoneFrom(pos, source cube.Pos, tx *world.Tx) {
	b := tx.Block(pos)
	r, updatesRedstone := b.(world.RedstoneUpdater)
	legacy, updatesNeighbours := b.(world.NeighbourUpdateTicker)
	if !updatesRedstone && !updatesNeighbours {
		return
	}
	...
	tx.Redstone().WithUpdateSource(source, func() {
		...
		legacy.NeighbourUpdateTick(pos, source, tx)
	})
}
