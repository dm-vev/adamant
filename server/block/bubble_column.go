...
func (b BubbleColumn) EntityInside(pos cube.Pos, tx *world.Tx, e world.Entity) {
	...
	surface := above.OutOfBounds(tx.Range())
	if blockAbove, loaded := tx.BlockLoaded(above); loaded {
		surface = isAir(blockAbove)
	}
	if b.DragDown {
...
// updateBubbleColumn creates, redirects, or removes consecutive bubble column blocks above pos. It stops if a chunk is
// not ready so that neighbour propagation never loads or generates terrain.
func updateBubbleColumn(pos cube.Pos, tx *world.Tx) {
	below, loaded := tx.BlockLoaded(pos.Side(cube.FaceDown))
	if !loaded {
		return
	}
	dragDown, active := bubbleColumnDirection(below)
	for !pos.OutOfBounds(tx.Range()) {
		current, loaded := tx.BlockLoaded(pos)
		if !loaded {
			return
		}
