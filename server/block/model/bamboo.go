...
// BBox ...
func (b Bamboo) BBox(pos cube.Pos, _ world.BlockSource) []cube.BBox {
	pixels := 2.0
	if b.Thick {
		pixels = 3.0
	}
	...
	box := cube.Box(0.5-radius, 0, 0.5-radius, 0.5+radius, 1, 0.5+radius)
	return []cube.BBox{box.Translate(b.randomlyModifyPosition(pos))}
}
...
// randomToFloat32 converts random long to float in [0, 1).
func (Bamboo) randomToFloat32(random uint64) float32 {
	return float32(random>>40) * (1.0 / 16777216.0)
}

// calculateOffsetValue calculates offset value with quantization to discrete steps.
...
func (b Bamboo) randomlyModifyPosition(pos cube.Pos) mgl64.Vec3 {
	...
	// Consume a random value between X/Z to de-correlate the axes.
	prng.Next()
...
// FaceSolid ...
func (Bamboo) FaceSolid(cube.Pos, cube.Face, world.BlockSource) bool {
	return false
}
