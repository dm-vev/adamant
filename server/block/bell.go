...
// UseOnBlock ...
func (b Bell) UseOnBlock(pos cube.Pos, face cube.Face, _ mgl64.Vec3, tx *world.Tx, user item.User, ctx *item.UseContext) (used bool) {
	...
	if face == cube.FaceUp {
		...
		if _, ok := tx.Block(pos.Side(cube.FaceUp)).Model().(model.Solid); !ok {
			return false
		}
		b.Attach = HangingBellAttachment()
	} else {
		if _, ok := tx.Block(pos.Side(face.Opposite())).Model().(model.Solid); !ok {
			return false
		}
		b.Facing = face.Direction()
		b.Attach = WallBellAttachment()
		if _, ok := tx.Block(pos.Side(face)).Model().(model.Solid); ok {
...
func (b Bell) NeighbourUpdateTick(pos, _ cube.Pos, tx *world.Tx) {
	...
	for _, supportFace := range supportFaces {
		if _, ok := tx.Block(pos.Side(supportFace)).Model().(model.Solid); !ok {
...
// Activate ...
func (b Bell) Activate(pos cube.Pos, _ cube.Face, tx *world.Tx, u item.User, _ *item.UseContext) bool {
...
// EntityInside ...
func (b Bell) EntityInside(pos cube.Pos, tx *world.Tx, e world.Entity) {
	if ejector, ok := e.H().Type().(EntityEjector); ok {
		ejector.EntityEject(e, pos, tx)

		// Always ring the bell on its facing direction, even if the entity is ejected in a different direction.
		b.Ring(pos, b.Facing.Face(), tx)
	}
}
...
func (b Bell) Ring(pos cube.Pos, face cube.Face, tx *world.Tx) {
