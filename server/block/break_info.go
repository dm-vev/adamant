...
// BreaksInstantly checks if the block passed has zero hardness and therefore breaks without consuming tool
// durability. When an item stack is provided, it also preserves Adamant's tool-specific instant-break check.
func BreaksInstantly(b world.Block, stacks ...item.Stack) bool {
	breakable, ok := b.(Breakable)
	if !ok {
		return false
	}
	...
	return t.BaseMiningEfficiency(b)+efficiencyVal >= hardness*30
}
