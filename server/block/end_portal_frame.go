...
// Model ...
func (EndPortalFrame) Model() world.BlockModel {
...
// InsertEndPortalEye inserts an eye and notifies comparators, or returns false if the frame already held one.
func (f EndPortalFrame) InsertEndPortalEye(pos cube.Pos, tx *world.Tx) bool {
	if f.Eye {
		return false
	}
	f.Eye = true
