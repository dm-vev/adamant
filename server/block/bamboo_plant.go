...
// RandomTick ...
func (b Bamboo) RandomTick(pos cube.Pos, tx *world.Tx, _ *rand.Rand) {
	if b.Ready {
...
func (b Bamboo) grow(pos cube.Pos, amount int, maxHeight int, tx *world.Tx) bool {
	...
	for i, bl := range newBlocks {
		tx.SetBlock(pos.Sub(cube.Pos{0, i - amount}), bl, nil)
	}
	return true
}

// allBamboos returns all possible states of a bamboo block.
