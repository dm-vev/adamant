...
func (s *Session) closeCurrentContainer(tx *world.Tx) {
	...
	if ent := s.openedEntity.Load(); ent != nil {
		if e, ok := (*ent).Entity(tx); ok {
...
// StartUsingItem notifies the client that its local player started using the held item.
...
// StopUsingItem notifies the client that its local player stopped using the held item.
func (s *Session) StopUsingItem() {
	s.writePacket(&packet.PlayerAction{
		EntityRuntimeID: selfEntityRuntimeID,
