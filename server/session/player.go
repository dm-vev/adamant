...
func (s *Session) sendRecipes() {
	var (
		...
		userDataShapelessRecipes []protocol.ShulkerBoxRecipe
	...
	for index, i := range recipe.Recipes() {
		...
		case recipe.UserDataShapeless:
			userDataShapelessRecipes = append(userDataShapelessRecipes, protocol.ShulkerBoxRecipe{ShapelessRecipe: protocol.ShapelessRecipe{
				RecipeID:        uuid.New().String(),
				Priority:        int32(i.Priority()),
				Input:           stacksToIngredientItems(s.br, i.Input()),
				Output:          stacksToRecipeStacks(s.br, i.Output()),
	...
	s.writePacket(&packet.CraftingData{
		...
		ShulkerBoxRecipes:            userDataShapelessRecipes,
...
// ResyncInventory resends the main and off-hand inventories after an unfulfilled client prediction.
	...
	return func(slot int, before, after item.Stack) {
		...
		c.H().Do(func(tx *world.Tx, e world.Entity) {
			...
			if !after.Empty() && inTransaction {
				tx.PlaySound(entity.EyePosition(e), sound.EquipItem{Item: after.Item()})
			}
			tx.ReleaseViewers(viewers)
		})
	}
