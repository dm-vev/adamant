...
func (p *Player) Hurt(dmg float64, src world.DamageSource) (float64, bool) {
	...
	if holding, using := p.Blocking(); holding && using && src.ReducedByArmour() {
...
func (p *Player) respawn(f func(p *Player)) {
		...
		if bed, ok := blk.(block.Bed); ok {
			if bed.Head {
				...
				if w == currentWorld {
					...
					<-w.Exec(func(tx *world.Tx) {
						if f, ok := tx.Block(footPos).(block.Bed); ok {
							foot = f
							found = true
						}
					})
	...
	if respawnWorld == defaultWorld && respawnPos == defaultWorld.Spawn() && anchorWorld == nil && bedWorld == nil {
		anchorObstructed = true
	}
	...
	if bedWorld != nil {
		...
		if bedObstructed {
			bedWorld.SetPlayerSpawn(p.UUID(), bedWorld.Spawn())
		}
	}
...
// StopSleeping wakes the player up if they are currently sleeping.
func (p *Player) StopSleeping(tx *world.Tx) {
	if !p.sleeping {
		return
	}
	if tx != nil {
		tx.World().RemoveSleepingPlayer(p.UUID())
	}
...
// Sleeping reports if the player is currently sleeping.
func (p *Player) Sleeping() bool {
	return p.sleeping
}

// BedPosition returns the position of the bed the player is sleeping in, if any.
func (p *Player) BedPosition() (cube.Pos, bool) {
	if !p.sleeping {
		return cube.Pos{}, false
	}
	return p.sleepBed, true
}
...
func (p *Player) requiredSleepers(tx *world.Tx) int {
	total := 0
	tx.Players()(func(e world.Entity) bool {
		if pl, ok := e.(*Player); ok && pl.GameMode().Visible() {
			total++
		}
		return true
	})
	percentage := tx.World().PlayersSleepingPercentage()
	if percentage <= 0 {
		percentage = 100
	}
	required := int(math.Ceil(float64(total) * float64(percentage) / 100))
	if required < 1 {
		required = 1
	}
	return required
...
func (p *Player) tryAdvanceNight(tx *world.Tx) {
	required := p.requiredSleepers(tx)
	if tx.World().SleepingPlayerCount() < required {
		return
	}
...
func bedSleepPosition(pos cube.Pos, facing cube.Direction) mgl64.Vec3 {
	head := pos.Side(facing.Face())
...
func (p *Player) SetBlockingDelay(duration time.Duration) {
	...
	p.blockingDelayTicks = int64(duration.Seconds() * 20)
	if blocking {
		p.updateState()
	}
}
...
func (p *Player) AttackEntity(e world.Entity) bool {
	...
	if s, ok := i.Enchantment(enchantment.Sharpness); ok {
		...
		viewers, release := borrowViewers(p.tx, living.Position())
...
func (p *Player) tryShieldBlock(dmg float64, totalDamage float64, src world.DamageSource) bool {
	holding, using := p.Blocking()
	if !holding || !using || !src.ReducedByArmour() {
		return false
	}

	if attack, ok := src.(entity.AttackDamageSource); ok {
		if attack.Attacker != nil {
			diff := p.Position().Sub(attack.Attacker.Position())
			diff[1] = 0
	...
	if attack, ok := src.(entity.AttackDamageSource); ok {
		if attacker, ok := attack.Attacker.(entity.Living); ok {
			attacker.KnockBack(p.Position(), 0.5, 0.4)
		}
	...
	if dmg >= 3 {
		...
		if _, ok := held.Item().(item.Shield); ok {
			p.SetHeldItems(p.damageItem(held, damage), other)
		} else {
			p.SetHeldItems(held, p.damageItem(other, damage))
		}
	}
	return true
}
...
func (p *Player) viewers() ([]world.Viewer, bool) {
	viewers, ok := borrowViewers(p.tx, p.Position())
	...
	if !ok {
		if p.s != nil {
			return []world.Viewer{p.s}, false
		}
		return nil, false
	}
	var s world.Viewer = p.session()
	if slices.Index(viewers, s) == -1 && p.s != nil {
		viewers = append(viewers, p.s)
	}
	return viewers, true
}

func borrowViewers(tx *world.Tx, pos mgl64.Vec3) (viewers []world.Viewer, ok bool) {
	return txguard.Value(tx, func() []world.Viewer {
		return tx.Viewers(pos)
	})
}

func releaseBorrowedViewers(tx *world.Tx, viewers []world.Viewer, ok bool) {
	if !ok || tx == nil {
		return
	}
	txguard.Run(tx, func() {
