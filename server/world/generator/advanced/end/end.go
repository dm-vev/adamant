...
type End struct {
	...
	noiseGen5     *mc112.NoiseOctaves // Unused, but instantiated to match Java RNG consumption.
	noiseGen6     *mc112.NoiseOctaves // Unused, but instantiated to match Java RNG consumption.
	islandNoise   *mc112.NoiseSimplex
...
// New creates an End generator using the seed provided.
func New(seed int64) *End {
	return NewEnd(seed)
}
...
func NewEnd(seed int64) *End {
	...
	g := &End{
		...
		noiseGen5:     mc112.NewNoiseOctaves(r, 10),
		noiseGen6:     mc112.NewNoiseOctaves(r, 16),
		islandNoise:   mc112.NewNoiseSimplex(r),
...
func shuffleInts(r *mc112.Rand, list []int) {
...
func (g *End) populate(chunkX, chunkZ int, c *chunk.Chunk, r *mc112.Rand) {
...
func (g *End) generateEndIsland(c *chunk.Chunk, centreX, centreY, centreZ int, r *mc112.Rand) {
...
func (g *End) generateChorusPlant(c *chunk.Chunk, x, y, z int, r *mc112.Rand, maxDistance int) {
	g.setBlockRuntimeID(c, x, y, z, g.chorusPlantRID)
	g.growChorusRecursive(c, x, y, z, r, x, z, maxDistance, 0)
}

func (g *End) growChorusRecursive(c *chunk.Chunk, x, y, z int, r *mc112.Rand, originX, originZ, maxDistance, depth int) {
	i := int(r.Intn(4) + 1)
	if depth == 0 {
		i++
	}
...
func randomHorizontal(r *mc112.Rand) (dx, dz, opposite int) {
	switch r.Intn(4) {
	case 0:
