...
func lerp(t, t1, t2, q1, q2 float64) float64 {
	return ((t2-t)/(t2-t1))*q1 + ((t-t1)/(t2-t1))*q2
}
...
func (s *simplex) OctaveNoise3D(xin, yin, zin float64, normalized bool) (result float64) {
	...
	xin, yin, zin = xin*s.expansion, zin*s.expansion, yin*s.expansion

	for i := 0; i < s.octaves; i++ {
		result += s.Noise3D(xin*freq, zin*freq, yin*freq) * amp
		freq *= 2.0
		maxAmp += amp
		amp *= s.persistence
	}
